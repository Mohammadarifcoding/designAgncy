...
const Navbar = () => {
	...
	return (
		<div
			className={`md:p-1.5 bg-gradient-to-b from-black to-gray-800 text-white  rounded-full inner-shadow bottom-5 z-50 fixed left-5 md:left-1/2 duration-300 md:-translate-x-1/2  ${
			...
			<div className="flex flex-col md:flex-row items-center gap-6 ">
				...
				<div className={`flex items-center flex-col md:flex-row py-4 px-2 md:p-0 gap-2 ${isOpen ? "scale-0 absolute" : "scale-100"}`}>
					{navMenu?.map((item, idx) => (
						<div key={idx} className="text-center text-white font-medium text-sm  hover:bg-blue-400 px-3 py-2 rounded-full transition-all duration-500 whitespace-nowrap">
							{item}
						</div>
					))}
				</div>
